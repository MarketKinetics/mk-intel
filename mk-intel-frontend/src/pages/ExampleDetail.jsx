...
import { examples as examplesApi } from '../api/client'

// Universal safe string extractor - handles any field shape the LLM might produce
function s(val) {
...
// Universal confidence case labels - fixed by pipeline spec, not dataset-specific
...
// Tab order: Exec summary first, then Scoring (renamed from Overview)
const SECTIONS = [
...
function ScoreBadge({ score, rank }) {
  const colors = {
    1: 'bg-teal-accent/15 text-teal-dark border-teal-accent/30',
    2: 'bg-green-50 text-green-700 border-green-200',
    3: 'bg-amber-50 text-amber-700 border-amber-200',
    4: 'bg-gray-100 text-slate border-gray-200',
  }
  return (
    <span className={`text-xs px-2.5 py-1 rounded-full font-medium border ${colors[rank] || colors[4]}`}>
      #{rank} · {score.toFixed(3)}
    </span>
  )
}

// Radar/spider chart for dimension scores using SVG
    ...
    <div className="flex items-center gap-6">
      <svg width="160" height="160" viewBox="0 0 160 160">
        ...
        <polygon
          ...
          fill={fillColor} stroke={color} strokeWidth="1.5"
        />
        {points.map((p, i) => (
        ...
        {points.map((p, i) => (
          <text key={i} x={p.lx} y={p.ly} textAnchor="middle" dominantBaseline="middle"
            fontSize="9" fill="#5C6B7A" fontFamily="Inter, system-ui">
            {dims[i].label}
          </text>
        ))}
      </svg>
...
function TARCard({ tar, ranking, slug, sobjLabels }) {
  ...
  // Universal: get SOBJ label from the passed sobjLabels map, fallback to raw ID
  const sobjLabel = sobjLabels?.[tar.sobj_id] || tar.sobj_id

  // Universal: confidence case plain English
  const confidenceLabel = CONFIDENCE_LABELS[tar.confidence] || `Case ${tar.confidence}`
  ...
  async function handleClick() {
    ...
    setLoadingTar(true)
    try {
      const r = await examplesApi.getTar(slug, tar.tar_id)
      setTarData(r.data)
      setExpanded(true)
    } catch (e) {
      console.error('TAR load error:', e)
    } finally {
      setLoadingTar(false)
    }
  }
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              {rank && score && <ScoreBadge score={score} rank={rank} />}
              {rank === 1 && (
                <span className="text-xs bg-teal-dark text-white px-2 py-0.5 rounded-full font-medium">First priority</span>
              )}
            </div>
            <div className="text-base font-medium text-ink mb-1">{tar.audience_name || tar.ta_id}</div>
            <div className="text-xs text-slate">{sobjLabels?.[tar.sobj_id] || tar.sobj_id}</div>
          </div>
      ...
      {expanded && tarData && (
        <div className="border-t border-gray-100">
          <div className="flex items-center justify-between px-5 pt-3 pb-0 border-b border-gray-100">
            ...
            <Link
              to={`/examples/${slug}/tars/${tar.tar_id}`}
          ...
          <div className="p-5">
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {[
                    ...
                    {
                      label: 'Confidence',
                      val: CONFIDENCE_LABELS[tarData.confidence_case] || `Case ${tarData.confidence_case}`
                    },
                ...
                {tarData.effectiveness?.sobj_impact?.description && (
                  <div>
                    <div className="text-xs font-medium text-slate uppercase tracking-widest mb-2">Campaign impact</div>
                    <p className="text-xs text-slate leading-relaxed">{s(tarData.effectiveness.sobj_impact.description)}</p>
                  </div>
                )}
            ...
            {activeSection === 'actions' && tarData.narrative_and_actions?.recommended_actions && (
              ...
              <p className="text-xs text-slate italic mb-3">These recommendations are channel and budget agnostic. Detailed campaign planning — including media selection, spend allocation, and frequency — will be available in MK Campaign.</p>
            ...
            {activeSection === 'assessment' && tarData.assessment && (
              <div className="space-y-4">
                ...
                {tarData.assessment.metrics?.length > 0 && (
                  <div>
                    <div className="text-xs font-medium text-slate uppercase tracking-widest mb-2">Success metrics</div>
                    <div className="space-y-2">
                      {tarData.assessment.metrics.map((m, i) => (
                        <div key={i} className="bg-surface rounded-lg p-3">
                          <div className="text-xs font-medium text-ink mb-1">
                            {typeof m === 'string' ? m : s(m.metric_name) || `Metric ${i + 1}`}
                          </div>
                          {typeof m === 'object' && m.definition && (
                            <p className="text-xs text-slate leading-relaxed">{s(m.definition)}</p>
                          )}
                          {typeof m === 'object' && m.success_threshold && (
                            <div className="text-xs text-teal-dark mt-1 font-medium">Target: {s(m.success_threshold)}</div>
                          )}
                        </div>
                      ))}
                    </div>
...
export function ExampleDetail() {
  ...
  const [sobjLabels, setSobjLabels] = useState({})

  useEffect(() => {
    examplesApi.get(slug)
      .then(async r => {
        setData(r.data)
        const sobjs = Object.keys(r.data.rankings || {})
        if (sobjs.length) setSelectedSobj(sobjs[0])
        // Fetch first TAR per SOBJ to get the sobj_statement
        const sobjIds = [...new Set((r.data.tars || []).map(t => t.sobj_id).filter(Boolean))]
        const labels = {}
        await Promise.all(sobjIds.map(async (sobjId) => {
          const firstTar = r.data.tars.find(t => t.sobj_id === sobjId)
          if (firstTar) {
            try {
              const tarRes = await examplesApi.getTar(r.data.slug, firstTar.tar_id)
              labels[sobjId] = tarRes.data.sobj_statement || sobjId
            } catch {
              labels[sobjId] = sobjId
            }
          }
        }))
        setSobjLabels(labels)
      })
  ...
  const sortedTars = [...(data.tars || [])].sort((a, b) => {
    const ra = getRankingForTar(a.tar_id)?.rank ?? 99
    const rb = getRankingForTar(b.tar_id)?.rank ?? 99
    return ra - rb
  })

  // Build universal SOBJ selector labels from TAR sobj_id
  // Filter tars for current sobj if multiple
  const tarsForSobj = selectedSobj
    ? sortedTars.filter(t => t.sobj_id === selectedSobj)
    : sortedTars

  // Short description: first sentence only, universal
    ...
    <div className="min-h-screen" style={{ background: '#F8F7F4' }}>
      <div style={{ background: 'radial-gradient(ellipse at 50% -20%, #102847 0%, #0A1628 60%)' }}
        ...
        <div className="max-w-4xl mx-auto">
          <Link to="/examples" className="inline-flex items-center gap-1.5 text-xs text-white/40 hover:text-white/70 transition-colors mb-6">
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className="text-xs font-medium text-teal-accent/60 uppercase tracking-widest">{data.sector}</span>
                {data.zip_enrichment && <span className="text-xs text-white/40 bg-white/10 px-2 py-0.5 rounded">ZIP enriched</span>}
              </div>
              <h1 className="text-2xl sm:text-3xl font-medium text-white/95 tracking-tight mb-2">{data.name}</h1>
              <p className="text-sm text-white/45 max-w-xl">{shortDesc}</p>
            </div>
            <div className="flex gap-6">
              {[
                { val: data.ta_count, label: 'segments' },
                { val: data.tar_count, label: 'reports' },
                { val: data.sobj_count, label: 'objectives' },
              ].map(m => (
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        {sobjs.length > 1 && (
          <div className="flex gap-2 mb-6 flex-wrap">
            {sobjs.map(sobjId => {
              // Use the statement from first matching tar if available
              const matchingTar = data.tars?.find(t => t.sobj_id === sobjId)
              const label = matchingTar?.sobj_statement || sobjId
        ...
        {currentRankings.length > 0 && (
          <div className="bg-white border border-gray-200/60 rounded-xl p-5 mb-6">
            <div className="text-xs font-medium text-slate uppercase tracking-widest mb-4">
              Rankings — {data.tars?.find(t => t.sobj_id === selectedSobj)?.sobj_statement || selectedSobj}
            </div>
            <div className="space-y-3">
              {currentRankings.map(r => {
                const tar = data.tars.find(t => {
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="text-xs font-medium text-slate uppercase tracking-widest">Target audience reports</div>
          <a
            href={`${import.meta.env.VITE_API_URL || 'https://web-production-7ec13.up.railway.app'}/examples/${slug}/tars/export/${selectedSobj}`}
            target="_blank"
        ...
        <div className="space-y-3">
          {tarsForSobj.map(tar => (
            <TARCard key={tar.tar_id} tar={tar} ranking={getRankingForTar(tar.tar_id)} slug={slug} sobjLabels={sobjLabels} />
          ))}
        </div>
      </div>
    </div>
